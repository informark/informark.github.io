...
function temContextoDeProduto(texto) {
  ...
  if (produto && produto !== "Outro" && produto !== "Perfume" && produto !== "Bebida" && produto !== "Veículo") return true;
...
function extrairModeloIphoneDefinitivo(texto) {
  ...
  if ((/\biphone\s*air\b/.test(t) || /\bair\b/.test(t)) && /\biphone\b/.test(t) && !/\b(8|11|12|13|14|15|16|17)\b/.test(t)) return "Air";
  ...
  const temSufixoIphone = /\b(pro\s*max|promax|\bpm\b|pro|plus|mini|max|air)\b/.test(t);
...
if (reNovo.test(t)) return "Novo";
  ...
  if ((produto || "") === "iPhone") {
    ...
    if (/\b(c\/\s*nf|nota\s*fiscal|nf\b|anatel|c\s*nota)\b/i.test(t)) return "Novo";
    if (/\bcx\b/i.test(t) && !/\b(novo|lacrad|selad|nf|anatel)\b/i.test(t)) return "Seminovo";
  ...
  if (prod === "apple watch") {
    if (/\b(ultra\s*[123]?|series\s*\d{1,2}|s\d{1,2})\b/i.test(t) && !/\b(usado|seminovo|vitrine|bateria\s*\d)/i.test(t)) return "Novo";
  }

  if (["perfume", "bebida", "conectividade"].includes(prod)) return "Novo";

  if (/\b(tem\s*caixa)\b/i.test(t)) return "Seminovo";

  if (/\b(garantia|nf\b|nota\s*fiscal|c\/?\s*nf|anatel|c\s*nota)\b/i.test(t) && !/\b(usado|seminovo|vitrine)\b/i.test(t)) return "Novo";

  if (prod === "iphone") {
    if (/\b(pro\s*max|pro|air)\b/i.test(t) && /\b\d{2,3}\s*g?b?\b/i.test(t)) return "Novo";
    if (/\b1[1-7]\b/.test(t) && /\b(64|128|256|512|1\s*tb)\b/i.test(t) && !/\b(usado|seminovo|vitrine)\b/i.test(t)) return "Novo";
    if (/\b(oferta|promo)/i.test(t)) return "Novo";
  }

  if (["acessório", "acessorio"].includes(prod)) return "Novo";

  if (prod === "console") {
    if (/\b(ps5|ps4|playstation|xbox|switch)\b/i.test(t) && !/\b(usado|seminovo|vitrine)\b/i.test(t)) return "Novo";
  }

  if (prod === "xiaomi") {
    if (/\b(poco|redmi|amazfit|mi\s*\d)\b/i.test(t) && !/\b(usado|seminovo|vitrine)\b/i.test(t)) return "Novo";
  }

  if (prod === "garmin") return "Novo";

  if (prod === "jbl") return "Novo";

  if (prod === "tv") return "Novo";
...
function detectarProduto(texto) {
  ...
  if (/\b(perfume|colonia|fragr[aâ]ncia|eau\s*de\s*(toilette|parfum))\b/i.test(t)) return "Perfume";
  if (/🧴/.test(raw)) return "Perfume";
  if (/\b(lattafa|rasasi|afnan|bharara|montblanc|victoria.s\s*secret)\b/i.test(t)) return "Perfume";

  if (/\b(vinho|espumante|champagne|cerveja\s*artesanal)\b/i.test(t)) return "Bebida";
  if (/🍷/.test(raw) && !/\b(iphone|ipad|macbook|airpods|jbl|samsung|xiaomi)\b/i.test(t)) return "Bebida";

  if (/\b(carro|veiculo|suv|sedan|hatch|pickup|caminhonete)\b/i.test(t)) return "Veículo";
  if (/\b(creta|onix|hb20|corolla|civic|tracker|kicks|renegade|compass|toro|hilux|s10|saveiro)\b/i.test(t) && /\b(flex|gasolina|diesel|km|ano|motor)\b/i.test(t)) return "Veículo";
  if (/🚘|🚗|🏎/.test(raw)) return "Veículo";
  if (/\bmotor\s*\d+\.\d+\b/i.test(t) && /\b(flex|gasolina|diesel|km)\b/i.test(t)) return "Veículo";
...
const series = m ? `S${m[1]}` : "";
  ...
  if (produto === "Samsung") {
    ...
    if (m) return `Galaxy ${m[1].toUpperCase()}${m[2] ? " " + m[2].toUpperCase() : ""}`;
    m = tx2.match(/\bgalaxy\s*(z\s*(?:flip|fold))\s*(\d)?\b/i);
    if (m) return `Galaxy ${m[1].replace(/\s+/g, " ")}${m[2] ? " " + m[2] : ""}`;
  ...
  if (produto === "Perfume") {
    ...
    for (const [re, nome] of marcas) {
      if (re.test(tx2)) {
        const semMarca = tx2.replace(re, "").replace(/[*_~🧴💰🔥]/g, "").replace(/r\$\s*[\d.,]+/gi, "").replace(/\s+/g, " ").trim();
        const palavras = semMarca.split(/\s+/).filter(p => p.length > 2).slice(0, 4).join(" ");
        return palavras ? `${nome} ${palavras}` : nome;
      }
    }
    let semPreco = tx2.replace(/r\$\s*[\d.,]+/gi, "").replace(/[🧴💰🔥*_~|]/g, " ").replace(/\s+/g, " ").trim();
    const palavras = semPreco.split(/\s+/).filter(p => p.length > 2).slice(0, 5).join(" ");
    return palavras || "Perfume (não identificado)";
  }

  if (produto === "Bebida") {
    let semPreco = (t || "").replace(/r\$\s*[\d.,]+/gi, "").replace(/[🍷💰🔥*_~|]/g, " ").replace(/\s+/g, " ").trim();
    const palavras = semPreco.split(/\s+/).filter(p => p.length > 2).slice(0, 5).join(" ");
  ...
  if (produto === "Notebook") {
    ...
    let modelo = "";
    const m = tx2.match(/\b(inspiron|aspire|vivobook|pavilion|latitude|ideapad)\s*(\d+)?\b/i);
...
function extrairItensDeLista(texto) {
  ...
  else if (titulo === "perfumes" || titulo === "perfume") contextoProduto = "Perfume";
  else if (titulo === "bebidas" || titulo === "vinhos") contextoProduto = "Bebida";
